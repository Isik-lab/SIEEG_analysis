...
class FeatureLatencyDist:
    ...
    def load_data(self):
        ...
        df = df.loc[df.feature == self.feature].reset_index()
        return df
...
def main():
    ...
    parser.add_argument('--out_dir', '-o', type=str, help='directory for outputs',
                        default='/home/emcmaho7/scratch4-lisik3/emcmaho7/SIEEG_analysis/data/interim/FeatureLatencyDist')
    parser.add_argument('--input_dir', '-i', type=str, help='directory for outputs',
                        default='/home/emcmaho7/scratch4-lisik3/emcmaho7/SIEEG_analysis/data/interim/PlotNuisanceFeatureDecoding')
    parser.add_argument('--feature', '-f', type=str, default='expanse',
                        help='feature to calculate dist for')
    parser.add_argument('--n_resamples', '-n', type=int, default=1000,
                        help='number of time to resample the participants')
    args = parser.parse_args()
    FeatureLatencyDist(args).run()
